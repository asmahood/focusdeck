...
/**
 * Custom hook for fetching and managing GitHub pull requests with cursor-based pagination
 *
 ...
 * @example
 * ```tsx
 * const { items, totalCount, pageInfo, isLoading, error, loadMore } = usePullRequests({
 *   initialData
 * });
 *
 ...
 * ```
 */
export function usePullRequests({ initialData }: UsePullRequestsProps) {
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      const url = pageInfo.endCursor
        ? `/api/pull-requests?cursor=${encodeURIComponent(pageInfo.endCursor)}`
        : "/api/pull-requests";
