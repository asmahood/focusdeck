...
interface UseIssuesAssignedProps {
  initialData: FetchResult;
}

export function useIssuesAssigned({ initialData }: UseIssuesAssignedProps) {
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      const url = pageInfo.endCursor
        ? `/api/issues/assigned?cursor=${encodeURIComponent(pageInfo.endCursor)}`
        : "/api/issues/assigned";
