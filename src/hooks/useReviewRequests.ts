...
interface UseReviewRequestsProps {
  initialData: FetchResult;
}

/**
 * Custom hook for fetching and managing GitHub review requests with cursor-based pagination
 *
 ...
 * @example
 * ```tsx
 * const { items, totalCount, pageInfo, isLoading, error, loadMore } = useReviewRequests({
 *   initialData
 * });
 *
 ...
 * ```
 */
export function useReviewRequests({ initialData }: UseReviewRequestsProps) {
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      const url = pageInfo.endCursor
        ? `/api/review-requests?cursor=${encodeURIComponent(pageInfo.endCursor)}`
        : "/api/review-requests";
